...
PROJECTNAME := $(shell git remote get-url origin 2>/dev/null | sed -E 's|.*/||; s|\.git$$||')
PROJECTNAME := $(if $(PROJECTNAME),$(PROJECTNAME),$(shell basename "$$(pwd)"))
PROJECTORG := $(shell git remote get-url origin 2>/dev/null | sed -E 's|\.git$$||; s|.*/([^/]+)/[^/]+$$|\1|')
PROJECTORG := $(if $(PROJECTORG),$(PROJECTORG),$(shell basename "$$(dirname "$$(pwd)")"))
...
# Format: "January 22, 2025 at 10:30:45"
...
# Docker - Set REGISTRY based on your platform
...
local: clean
	...
	# Build server binary (Linux - container native)
	...
	@if [ -d "src/client" ]; then \
		...
		$(GO_DOCKER) go build -ldflags "$(LDFLAGS)" -o $(BINDIR)/$(PROJECTNAME)-cli ./src/client; \
	fi

	# Build agent binary (if exists)
	@if [ -d "src/agent" ]; then \
		echo "Building $(PROJECTNAME)-agent..."; \
		$(GO_DOCKER) go build -ldflags "$(LDFLAGS)" -o $(BINDIR)/$(PROJECTNAME)-agent ./src/agent; \
	fi
...
dev:
	...
	@mkdir -p "$${TMPDIR:-/tmp}/$(PROJECTORG)" && \
		...
		docker run --rm \
			...
			-w /build \
			-e CGO_ENABLED=0 \
			golang:alpine go build -o /out/$(PROJECTNAME) ./src && \
		...
		if [ -d "src/client" ]; then \
			docker run --rm \
				...
				-w /build \
				-e CGO_ENABLED=0 \
				golang:alpine go build -o /out/$(PROJECTNAME)-cli ./src/client && \
		...
		if [ -d "src/agent" ]; then \
			docker run --rm \
				...
				-w /build \
				-e CGO_ENABLED=0 \
				golang:alpine go build -o /out/$(PROJECTNAME)-agent ./src/agent && \
